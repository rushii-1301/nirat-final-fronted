...
function Videos({ isDark, toggleTheme, sidebardata }) {
    ...
    const [selectedSharePeer, setSelectedSharePeer] = useState(null);
    ...
    const sendShare = async () => {
        if (!selectedSharePeer?.enrollment_number) {
            setShareError("Please select a student");
            return;
        }
        ...
        const videoId = params.id || passedVideo?.id;
        if (!videoId) {
            setShareError("Video id not found");
            return;
        }
        ...
        try {
            ...
            const res = await axios.post(
                ...
                {
                    peer_enrollment: selectedSharePeer.enrollment_number,
                    message: shareNote || null,
                },
                {
                    headers: {
                        accept: "application/json",
            ...
            if (res?.data?.status) {
                setShareSuccess(`Sent to ${selectedSharePeer.name || selectedSharePeer.enrollment_number}`);
                setTimeout(() => {
                    closeShareModal();
                }, 700);
            } else {
                setShareError(res?.data?.message || "Failed to share video");
            }
        } catch (err) {
            console.error("Failed to share video", err);
            setShareError(err?.response?.data?.message || "Failed to share video");
        } finally {
    ...
    const sendWatchProgress = async (videoId, elapsedSeconds) => {
        try {
            ...
            await axios.post(
                ...
                { watch_seconds: Math.floor(elapsedSeconds) },
                {
                    headers: {
                        accept: "application/json",
                        "Content-Type": "application/json",
    ...
    useEffect(() => {
        ...
        const sendWatchIfAny = () => {
            ...
            if (elapsedSeconds > 0) {
                ...
                sendWatchProgress(session.videoId, elapsedSeconds);
            }
        };
        ...
        const handlePlay = () => {
            ...
            if (videoId) {
                watchSessionRef.current = { startedAt: Date.now(), videoId };
            }
        };
                    ...
                    {loading ? (
                        ...
                        <div className="relative flex-1 min-h-0">
                            ...
                            {showShareModal && (
                                <>
                                    <div
                                        className={`fixed inset-0 z-40 ${isDark ? 'bg-black/60' : 'bg-black/40'}`}
                                        onClick={closeShareModal}
                                    />
                                    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                                        <div
                                            className={`w-full max-w-md rounded-xl border shadow-2xl ${isDark ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-zinc-200 text-zinc-900'}`}
                                            onClick={(e) => e.stopPropagation()}
                                        >
                                            <div className={`flex items-center justify-between px-4 py-3 border-b ${isDark ? 'border-zinc-800' : 'border-zinc-200'}`}>
                                                <div className="font-semibold">Share to classmate</div>
                                                <button
                                                    onClick={closeShareModal}
                                                    className={`h-8 w-8 inline-flex items-center justify-center rounded-md cursor-pointer transition ${isDark ? 'hover:bg-zinc-800 text-zinc-400' : 'hover:bg-zinc-100 text-zinc-600'}`}
                                            ...
                                            <div className="p-4">
                                                <input
                                                    ...
                                                    className={`w-full rounded-md border px-3 py-2 outline-none ${isDark ? 'bg-zinc-800 border-zinc-800 placeholder-zinc-500 focus:ring-2 focus:ring-[#2563EB] focus:border-transparent' : 'bg-white border-zinc-300 placeholder-zinc-400 focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent'}`}
                                                />
                                                    ...
                                                    {shareLoading ? (
                                                        ...
                                                        (sharePeers || [])
                                                            .filter((p) => {
                                                                ...
                                                                const e = String(p?.enrollment_number || '').toLowerCase();
                                                            ...
                                                            .map((peer) => (
                                                                <button
                                                                    ...
                                                                    type="button"
                                                                    onClick={() => setSelectedSharePeer(peer)}
                                                                    className={`w-full text-left px-3 py-2 rounded-md border mb-2 cursor-pointer transition ${isDark ? 'border-zinc-800 hover:bg-zinc-800' : 'border-zinc-200 hover:bg-zinc-50'} ${selectedSharePeer?.enrollment_number === peer.enrollment_number ? (isDark ? 'bg-zinc-800' : 'bg-zinc-50') : ''}`}
                                                                >
                                                                    <div className="font-medium truncate">{peer.name || peer.enrollment_number}</div>
                                                                    <div className={`text-xs mt-0.5 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>{peer.enrollment_number}</div>
                                                                </button>
                                                            ))
                                                    )}
                                                    {!shareLoading && (!sharePeers || sharePeers.length === 0) && (
                                                        <div className="py-6 text-center text-sm opacity-70">No students found</div>
                                                    )}
                                                </div>
                                                ...
                                                <textarea
                                                    ...
                                                    rows={2}
                                                    className={`mt-2 w-full resize-none rounded-md border px-3 py-2 outline-none ${isDark ? 'bg-zinc-800 border-zinc-800 placeholder-zinc-500 focus:ring-2 focus:ring-[#2563EB] focus:border-transparent' : 'bg-white border-zinc-300 placeholder-zinc-400 focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent'}`}
                                                />

                                                {shareError && <div className="mt-2 text-sm text-red-500">{shareError}</div>}
                                                {shareSuccess && <div className={`mt-2 text-sm ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>{shareSuccess}</div>}

                                                <div className="mt-4 flex items-center justify-end gap-2">
                                                    <button
                                                        onClick={fetchSharePeers}
                                                        disabled={shareLoading || sendingShare}
                                                        className={`h-9 px-3 rounded-md cursor-pointer transition ${isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-zinc-100 hover:bg-zinc-200'} ${(shareLoading || sendingShare) ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                    >
                                                        Refresh
                                                    </button>
                                                    <button
                                                        ...
                                                        className={`h-9 px-4 rounded-md cursor-pointer transition ${isDark ? 'bg-white text-zinc-900 hover:bg-zinc-200' : 'bg-zinc-900 text-white hover:bg-zinc-800'} ${sendingShare ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                    >
