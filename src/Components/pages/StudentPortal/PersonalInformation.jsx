...
function PersonalInformation({ isDark, toggleTheme, sidebardata }) {
        ...
        const fetchProfile = async () => {
            try {
                ...
                setProfile({
                    ...
                    photo: testUrls[0] || "", // Use first test URL
                });
            } catch (error) {
    ...
    const Field = ({ icon: Icon, label, value }) => (
        <div className="space-y-2">
            ...
            <div className={`flex items-center ${inputBg} ${inputText} border ${isDark ? 'border-zinc-700' : 'border-zinc-200'} rounded-lg h-12 px-3`}>
                <Icon className={`w-5 h-5 ${subText}`} />
                <span className="ml-3 text-sm font-medium truncate">{value}</span>
            </div>
        </div>
    );
                        ...
                        <div className={`relative border ${panelBg} rounded-2xl pt-20 p-6 md:pt-24 md:p-8 shadow-xl mt-16`}>
                            ...
                            <div className="absolute -top-16 left-1/2 -translate-x-1/2">
                                <div className="w-32 h-32 flex items-center justify-center">
                                    <div className={`w-28 h-28 rounded-full flex items-center justify-center shadow-md ${isDark ? 'bg-zinc-800' : 'bg-gray-100'}`}>
                                        {profile.photo ? (
                                            <img
                                                alt="avatar"
                                                src={profile.photo}
                                        ...
                                        <div className={`${profile.photo ? 'hidden' : 'flex'} items-center justify-center w-full h-full`}>
                            ...
                            <div className="space-y-8">
                                ...
                                <section>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                        <Field icon={User} label="Full Name" value={profile.first_name || ""} />
                                        <Field icon={Users} label="Father Name" value={profile.father_name || ""} />
                                        <Field icon={IdCard} label="Enrollment Number" value={profile.enrollment_number || ""} />
                                ...
                                <section className="pt-6 border-t border-zinc-800/80">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                        ...
                                        <Field icon={Building2} label="Division" value={profile.division || ""} />
                                        <Field icon={School} label="Class Head" value={profile.class_head || ""} />
                                        <Field icon={Mail} label="Email Address" value={profile.email || ""} />
                                    </div>
                                </section>
