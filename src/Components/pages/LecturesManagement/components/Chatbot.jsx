...
//     // Auto-scroll to bottom
//     useEffect(() => {
//         // Cleanup function: Jab component unmount hoga tab chalega
//         return () => {
//             if (recognitionRef.current) {
//                 recognitionRef.current.onend = null; // Callback hata do
//                 recognitionRef.current.stop();       // Mic band kar do
//             }
//         };
//     }, []);
//     const startVoiceRecognition = () => {
...
// import React, { useState, useRef, useEffect } from 'react';
// import { X, Mic, SendHorizontal, Bot } from 'lucide-react';
...
//     // ✅ FIX 1: Auto-scroll Logic (Ye wapas add kiya hai)
//     useEffect(() => {
//         if (chatContainerRef.current) {
//             // Thoda timeout diya taaki UI render hone ke baad scroll ho
//             setTimeout(() => {
//                 chatContainerRef.current.scrollTop = chatContainerRef.current.scrollHeight;
//             }, 100);
//         }
//     }, [messages]); // Jab bhi 'messages' change honge, ye chalega

//     // ✅ FIX 2: Mic Cleanup Logic (Ye bhi zaroori hai)
//     useEffect(() => {
//         return () => {
//             if (recognitionRef.current) {
...
//         recognition.onend = () => {
//             setMicStatus('idle');
//             // ✅ Fix: Current state value access karne me issue ho sakta hai closure ki wajah se, 
//             // lekin yahan hum handleSend call kar rahe hain jo state uthayega.
//             // Behtar hai user khud send dabaye ya hum check karein.
//             // Filhal manual send button safe hai, auto-send hata sakte ho agar glitch kare.
//         };
...
//     return (
//         <div className="fixed right-6 bottom-6 w-80 rounded-3xl border border-gray-200 shadow-2xl overflow-hidden flex flex-col z-50 bg-white h-96">
//             {/* Header */}
//             <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
//                 <h3 className="font-semibold text-sm text-gray-900">AI Assistant</h3>
//                 <button onClick={onClose} className="p-1.5 rounded-full hover:bg-gray-200 text-gray-600">
...
//             {/* Messages */}
//             <div ref={chatContainerRef} className="p-4 space-y-4 overflow-y-auto flex-1 scrollbar-none">
//                 {messages.map((msg) => (
//                     <div key={msg.id} className={`flex items-end gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
//                         {msg.sender === 'system' && (
//                             <div className="shrink-0 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200">
...
import React, { useState, useRef, useEffect } from 'react';
import { X, Mic, SendHorizontal, Bot } from 'lucide-react';
// Agar aapke paas MathText component hai to uncomment karein:
// import MathText from '../../../Tools/MathText'; 
...
function Chatbot({ messages, onSendMessage, onClose }) {
    ...
    // ✅ FIX 1: Auto-scroll Logic (Code 1 se liya gaya - Best approach)
    useEffect(() => {
        if (chatContainerRef.current) {
            setTimeout(() => {
                chatContainerRef.current.scrollTop = chatContainerRef.current.scrollHeight;
    ...
    const startVoiceRecognition = () => {
        ...
        recognition.onend = () => {
            setMicStatus('idle');
            // Humne yahan Auto-send NAHI lagaya (Code 1 approach), taaki user edit kar sake.
        };
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex items-end gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                        ...
                        <div className={`px-4 py-2 text-sm max-w-[80%] ${msg.sender === 'system'
                            ...
                            {/* Agar MathText available hai to ye use karein, warna normal rendering */}
                            {/* <MathText>{msg.text}</MathText> */}
                            {msg.text}
