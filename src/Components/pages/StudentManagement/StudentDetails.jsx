    ...
    const fetchStudentDetails = async (enrollmentNumber) => {
        ...
        try {
            ...
            if (response.data?.status === true && Array.isArray(response.data?.data?.students)) {
                ...
                if (foundStudent) {
                    setStudent({
                        name: `${foundStudent.profile_first_name || foundStudent.roster_first_name || ''} ${foundStudent.roster_last_name || ''}`.trim() || foundStudent.enrollment_number || 'Unknown',
                        enrollment: foundStudent.enrollment_number,
                        class: `${foundStudent.class_stream || foundStudent.std || ''} - Division ${foundStudent.profile_division || foundStudent.roster_division || ''}`,
                        father_name: foundStudent.father_name || '',
                        mobile_number: foundStudent.mobile_number || foundStudent.parents_number || '',
                        email: foundStudent.email || '',
                        class_head: foundStudent.class_head || '',
                        profile_complete: foundStudent.profile_complete || false,
                        profile_created_at: foundStudent.profile_created_at || '',
                        photo_path: foundStudent.photo_path || null,
                        purchases: foundStudent.purchases || []
                    });
                } else {
