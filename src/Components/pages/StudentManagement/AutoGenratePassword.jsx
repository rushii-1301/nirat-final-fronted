...
import { FileSpreadsheet, UploadCloud, Info, Download, FileUp } from "lucide-react";
...
function AutoGenratePassword({ theme, isDark, toggleTheme, sidebardata }) {
  ...
  const handleTemplateDownload = async () => {
    try {
      ...
      const data = [
        ['Enrollment Number', 'First Name', 'Last Name', 'Std', 'Div']
      ];
  ...
  throw new Error(errorData.message || errorData.detail || 'Failed to upload file');
      ...
      // Navigate to student list after successful upload
      setTimeout(() => {
    ...
    <div className={`flex h-screen transition-colors duration-300 ${pageClasses}`}>
      ...
      <div className="flex flex-col min-h-0 h-screen w-full md:ml-15 lg:ml-72 p-2 md:p-7 pb-0 transition-all duration-300">
        ...
        <main className="mt-2 flex-1 overflow-y-auto no-scrollbar flex items-center justify-center">
          <div
            className={`w-full max-w-lg rounded-2xl border shadow-sm px-6 py-6 ${cardClasses}`}
          >
            <div className="flex flex-col items-center text-center gap-2">
              <div
                className={`flex items-center justify-center w-14 h-14 rounded-full ${resolvedDark ? "bg-zinc-800" : "bg-zinc-100"
                  }`}
              >
                <FileSpreadsheet
              ...
              <h1 className="text-xl md:text-2xl font-semibold">Excel File Upload</h1>
              <p className={`text-xs md:text-sm ${subtleText}`}>
            ...
            <div className="mt-4 space-y-4 items-center">
              <button
                ...
                className={`w-full inline-flex items-center justify-center gap-2 rounded-md border px-4 py-2.5 text-sm font-medium cursor-pointer transition-colors ${resolvedDark
              ...
              <div
                ...
                className={`relative flex flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed px-4 py-10 cursor-pointer transition-colors ${resolvedDark
              ...
              <div
                className={`flex items-start gap-3 rounded-xl px-4 py-3 text-xs md:text-sm ${resolvedDark
                    ? "bg-zinc-900 border border-zinc-800"
                    : "bg-zinc-100 border border-zinc-200"
                  }`}
              >
                <div
                  className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-full border text-[10px] ${resolvedDark
                ...
                <div>
                  ...
                  <ul className="mt-1 space-y-0.5 list-disc list-inside">
                    ...
                    <li>
                      Example: <span className="font-mono">Ravi 1234</span>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                className={`inline-flex w-full items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold cursor-pointer disabled:opacity-60 ${resolvedDark
