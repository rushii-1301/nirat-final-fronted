...
function StudentsList({ theme, isDark, toggleTheme, sidebardata }) {
        ...
        try {
            ...
            if (response.data?.status === true && Array.isArray(response.data?.data?.students)) {
                ...
                const mapped = apiStudents.map((s, index) => ({
                    ...
                    name: `${s.profile_first_name || s.roster_first_name || ''} ${s.roster_last_name || ''}`.trim() || s.enrollment_number || 'Unknown',
                    enrollment_number: s.enrollment_number || s.profile_id?.toString() || index.toString(),
                    class_name: s.class_stream || s.std || '',
    ...
    useEffect(() => {
        if (location.state?.highlightedEnrollments) {
            ...
            // Remove highlight after 50 seconds
            setTimeout(() => {
                setHighlightedRows([]);
            }, 50000);
        }
                                ...
                                >
                                    <div className="space-y-5">
                                        ...
                                        <div className="flex justify-end gap-3 pt-2 border-t" style={{ borderColor: isDark ? '#27272a' : '#e4e4e7' }}>
                                            <button
                                                onClick={() => {
                                                    console.log("Edit button clicked for student:", s.enrollment_number, s.name);
                                                    console.log("Full student object:", s);

                                                    // Check if required fields are missing
                                                    if (!s.father_name || !s.mobile_number) {
                                                        handleerror("Cannot update student - Father Name and Mobile Number are required");
                                                        return;
                                                    }
